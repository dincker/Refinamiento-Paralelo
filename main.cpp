    ...
    if(rank==0){
        ...
        while(!ficheroEntrada.eof()){
            if(cuenta==0){
                ...
                Triangulo *tri = new Triangulo();
            ficheroEntrada.getline(tri->frase, 50);
        MPI_Send(&tri //REFERENCIA AL VECTOR DE ELEMENTOS A ENVIAR
             ,10000 //TAMAÑO del vector a enviar
        ...
        Triangulo *tri = new Triangulo();
        MPI_Recv(&tri //REFERENCIA AL VECTOR DONDE SE ALMACENA LO RECIVIDO
             ,10000 //NUMERO DE ELEMENTOS MAXIMO ARECIVIR
             ,MPI_PACKED //TIPO DE DATO QUE RECIVE
             ,0 //PID DEL PROCESO DE ORIGEN QUE SE RECIVE
             ,0 //ETIQUETA
             ,MPI_COMM_WORLD //COMUNICADOR POR EL QUE SECIVE
             ,MPI_STATUS_IGNORE); //ESTRUCTURA INFORMATIVA DEL ESTADO
                //cout << "frase: " << tri->frase << endl;
                tri->Split();
                tri->Cordenadas();
                tri->Angulos();
